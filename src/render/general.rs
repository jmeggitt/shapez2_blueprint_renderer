...
impl GeneralProgram {
    pub unsafe fn build(gl: &Gl) -> Result<Self, ShaderError> {
        ...
        // let vao = Vertex::build_vao(gl, program);
        check_for_errors(&gl);

        gl.UseProgram(program);
        check_for_errors(&gl);

        Ok(GeneralProgram { program, uniforms: GeneralProgramUniforms::from_program(gl, program) })
    }
}
...
impl GeneralProgramUniforms {
    unsafe fn from_program(gl: &Gl, program: GLuint) -> Self {
        ...
        let res = GeneralProgramUniforms {
    ...
    pub unsafe fn set_projection(&self, gl: &Gl, projection: &Mat4) {
        gl.UniformMatrix4fv(self.projection, 1, gl::FALSE, projection.as_ptr() as *const _);
        check_for_errors(&gl);
    }
